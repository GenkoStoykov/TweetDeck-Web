...
class Retweeter():
    ...
    def wait_on_action(self):
        ...
        if wait_time > 0:
            print(
                "Choosing time between %d and %d - waiting %d seconds before action" % (min_time, max_time, wait_time))
        ...
        for tweet in result["statuses"]:
            try:
                if (tweet["user"]["screen_name"] != self.BOT_CONFIG["TWITTER_HANDLE"] and
                    ...
                    self.TWITTER_CONNECTION.friendships.create(user_id=tweet["user"]["id"],
                                                               follow=False)
    ...
    def auto_follow_followers(self, count=None):
        ...
        for user_id in not_following_back:
            try:
                ...
                self.TWITTER_CONNECTION.friendships.create(user_id=user_id,
                                                           follow=False)
            except TwitterHTTPError as api_error:
                # quit on rate limit errors
                if "unable to follow more people at this time" in str(
                        api_error).lower():
                    print("You are unable to follow more people at this time. "
                          "Wait a while before running the bot again or gain "
                          "more followers.", file=sys.stderr)
    ...
    def auto_follow_followers_of_user(self, user_twitter_handle, count=100):
        ...
        followers_of_user = set(
            self.TWITTER_CONNECTION.followers.ids(screen_name=user_twitter_handle)["ids"][:count])
        do_not_follow = self.get_do_not_follow_list()
        ...
        for user_id in followers_of_user:
            try:
                ...
                # don't print "already requested to follow" errors - they're
                # frequent
                if "already requested to follow" not in str(
                        api_error).lower():
                    print("Error: %s" % (str(api_error)), file=sys.stderr)
    ...
    def auto_unfollow_nonfollowers(self, count=None):
        ...
        with open(self.BOT_CONFIG["ALREADY_FOLLOWED_FILE"],
                  "r") as in_file:
            for line in in_file:
        ...
        for user_id in not_following_back:
            if user_id not in self.BOT_CONFIG[
                "USERS_KEEP_FOLLOWING"]:
                self.wait_on_action()

                self.TWITTER_CONNECTION.friendships.destroy(
                    user_id=user_id)
    ...
    def auto_mute_following(self):
        ...
        following = self.get_follows_list()
        muted = set(
            self.TWITTER_CONNECTION.mutes.users.ids(
                screen_name=self.BOT_CONFIG[
                    "TWITTER_HANDLE"])["ids"])
        ...
        for user_id in not_muted:
            if user_id not in self.BOT_CONFIG[
                "USERS_KEEP_UNMUTED"]:
                self.TWITTER_CONNECTION.mutes.users.create(
                    user_id=user_id)
    ...
    def send_tweet(self, message):
        ...
        return self.TWITTER_CONNECTION.statuses.update(
            status=message)
    ...
    def getTopTweet(self, phrase, count=20, result_type="recent"):
        """
        Add users to list slug that are tweeting phrase.
        """
        ...
        for tweet in result["statuses"]:
            try:
                ...
                print("User %s " % (tweet["user"]["screen_name"]), file=sys.stdout)
